...
Node.js 24（engines 钉 24.18.0）+ TypeScript strict + 纯 ANSI 终端 UI（`src/tui/engine/`，零 React/Ink 渲染）+ DeepSeek V4 API。桌面端 `desktop/` 是独立 React 应用。

## Commands

```bash
npm run typecheck                                 # typecheck（跨会话共享闸门，与 tsc --noEmit 等价）
npm test                                          # all tests (node:test + assert/strict)
npm run build                                     # tsup bundle + 原生/wasm 载荷落位
npm run dev                                       # tsup --watch
node dist/cli/entry.js                            # launch（或 npm start）
```
