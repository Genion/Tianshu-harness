...
describe('resolveZenConfig — 默认物化与 fail-loud 校验', () => {
  it('undefined / null → 默认关闭（enabled false；读面/预算/分诊/appendix 仍物化默认）', () => {
  ...
  it('部分配置：缺省字段物化默认（enabled 缺省 = 关闭）', () => {
...
function makeController(
  ...
  const c = new ZenPhaseController(
    // 状态机 suite 默认测「zen 已启用」的相位行为——显式 enabled:false 的用例覆盖。
    resolveZenConfig({ enabled: true, ...over }),
    {
      isTopLevel: true,
...
describe('ZenPhaseController — 状态机', () => {
  ...
  it('promote 后 arm 不回 zen（不变量：promote 单向，晋升历史不被抹除）', () => {
    const { c } = makeController()
    c.arm()
    assert.equal(c.promote('tool'), true)
    assert.equal(c.currentPhase, 'full')

    // 历史缺陷：arm() 只挡 phase==='zen'，不挡"已晋升过"——重复 arm 会翻回 zen
    // 并把 promoteReason 清成 null（静默抹掉晋升历史）。类注释声称的不变量
    // 「promote 后不回 zen」当时没有代码守卫。
    c.arm()

    assert.equal(c.currentPhase, 'full', 'promote 后 arm 必须无效')
    assert.equal(c.isZen, false)
    assert.equal(c.lastPromoteReason, 'tool', '晋升历史不得被抹除')
  })
