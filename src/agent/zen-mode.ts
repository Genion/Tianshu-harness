...
/** 总开关。默认 false（2026-08-30 起默认关闭：读专注开局只对显式要的用户）。
   *  true → 新顶层会话启动即 arm（自动收窄读面）。false → 启动不 arm、恒全量面
   *  （不晋升、恒放行）——需要禅体验的用户在配置里显式开启，见 README「禅模式」。 */
  enabled?: boolean
   ...
   *  0 = 禁用超时晋升。默认 8。 */
  timeoutSteps?: number
  /** 首消息分诊：单行 && 长度 ≤ maxChars && 无附件 → 首轮请求前晋升（跳过禅）。
   *  默认 { enabled: true, maxChars: 80 }（2026-09-13 的「默认关」实验当日回退，
   *  原因见 defaultZenConfig 注释）。 */
...
/** 默认配置：关闭（读专注开局是显式 opt-in——2026-08-30 起默认不自动 zen，
 *  配置 `tools.zen.enabled: true` 或保持默认即可）。其余默认仍物化：minimal
 *  读面 + 8 turn 预算 + 短消息分诊 + appendix 收敛。
 *
 *  首消息分诊默认**开**：2026-09-13 有过一次「默认关」的实验，当日回退。当时的依据
 *  是长度判据与任务意图无关（「了解整个项目进度」8 字跑了 218 次工具调用却判为琐碎），
 *  但关掉后短消息一律进禅相位、受读面收窄，实测整体质量下降——放行少数理解型任务的
 *  代价，小于误关大量短请求的代价。判据不精确是真的，方向反了也是真的。 */
 ...
 * raw 为 undefined/null → 默认关闭配置（生产入口未显式配置即不启用 zen；
 * 2026-08-30 反转为 opt-in——此前未配置默认开启）。
 */
export function resolveZenConfig(raw: unknown): ResolvedZenConfig {
  ...
  // 缺省一律取自 defaultZenConfig()：默认值只能有单一真相源——2026-09-13 曾因
  // 内联 `?? true` 与 defaultZenConfig 分叉，改了默认值而生产路径不动（部分配置
  // 如 { enabled: true } 走的是内联缺省，不是 defaultZenConfig）。
  const defaults = defaultZenConfig()
  const enabled = obj.enabled ?? defaults.enabled
  if (typeof enabled !== 'boolean') {
    throw new ZenConfigError(`enabled 必须是布尔值（收到 ${JSON.stringify(enabled)}）`)
  }

  const faceModeRaw = obj.faceMode ?? defaults.faceMode
  ...
  const timeoutSteps = obj.timeoutSteps ?? defaults.timeoutSteps
  ...
  const appendixLean = obj.appendixLean ?? defaults.appendixLean
...
/** 首消息分诊纯判定：单行（无换行）&& 长度 ≤ maxChars → 视为琐碎请求，跳过禅。
 *  已知局限：长度与任务意图不可分——「你好」（2 字）与「了解整个项目进度」（8 字）
 *  落在同一档。2026-09-13 曾据此外推「默认关」并当日回退：放行少数理解型任务的
 *  代价，小于让所有短请求都受读面收窄的代价。 */
...
export class ZenPhaseController {
  ...
  arm(initialZenTurns = 0): void {
    ...
    // 不变量「promote 后不回 zen」的代码守卫：此前只挡 phase==='zen'，重复 arm 会把
    // 已晋升的会话翻回 zen 并清空 promoteReason（晋升历史被静默抹除）。
    if (this.phase === 'zen' || this.promoteReason !== null) return
